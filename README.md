...
`Dotyaml` is designed to help configure your application _at runtime_, and one
of the most effective places to do that is inside `config/runtime.exs` (available
since Elixir v1.11).

The `Dotyaml.source/2` function can accept a single file or a list of files.  When combined with `Config.config_env/0` it is easy to load up environment-specific config. A common setup in your `config/runtime.exs` would include a block like the following:
...
Unlike other packages, `Dotyaml` has no opinions about the names or locations of your `.env` files, you just need to pass their paths to `Dotyaml.source/2` or `Dotyaml.source!/2`.
...
And then define your variables in the file(s) to be sourced. `Dotyaml` has no opinions about what you name your files; `.env` is merely a convention.
...
See the `Dotyaml` module documentation on its functions.
...
Starting with `Dotyaml` v0.6.0, the precedence of system env variables over parsed `.env` files is not defined; the `:overwrite?` and `:vars` options are no longer supported in `Dotyaml.source/2` and `Dotyaml.source!/2`. Instead, the `source` functions now accept file paths OR maps: this makes the question of variable precedence something that must be explicitly listed. The `source` functions act more like `Map.merge/2`, accumulating values, always giving precedence to the righthand source.
